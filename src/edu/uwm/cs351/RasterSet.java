...
public class RasterSet{
	...
	private static final Comparator<Raster> BY_Y = new Comparator<Raster> () {
		public int compare(Raster r1, Raster r2) {
			//TODO: similar to the above comparator.
			...
			for(int i=0; i<r1.y(); i++)
				for(int j=0; j<r1.x(); j++) {
					c = P_COMP.compare(r1.getPixel(j, i), (r2.getPixel(j, i)));
					if (c != 0) return c;
				}
			return 0; //*
		}
	 ...
	 * @param lo all rasters in the subtree rooted at n must be bigger than this
	 * @param hi all rasters in the subtree rooted at n must be smaller this
	 * @return number of nodes in the subtree
	 */
	private int checkInRange(Node n, Raster lo, Raster hi) {
		// TODO: Implement this recursive method efficiently
		...
		if(n == null) return 0; //*
		if(n.raster == null) return reportNeg("n.raster is null!");
		if(lo != null && comp.compare(lo,n.raster) >= 0) return reportNeg("lo bound >= n.raster!");
		if(hi != null && comp.compare(hi,n.raster) <= 0) return reportNeg("hi bound <= n.raster!");
		if(checkInRange(n.left,lo,n.raster) == -1 || checkInRange(n.right,n.raster,hi) == -1) return -1; 
		return 1 + checkInRange(n.left,lo,n.raster) + checkInRange(n.right,n.raster,hi); //*
	}
	...
	public boolean xFirst(){
		...
		if(comp == BY_X) return false;
		comp = BY_X;
		resort();
		assert wellFormed() : "invariant false at the end of xFirst()";
		return true;
	}
	...
	public boolean yFirst(){
		...
		//use private helper methods to prevent duplicate code
		if(comp == BY_Y) return false; 
		comp = BY_Y;
		resort();
		assert wellFormed() : "invariant false at the end of yFirst()";
		return true;
	}
	
	private void resort() {
		//TODO: Implement this private helper method.
	...
	public Raster[] xRange(int lo, int hi) {
		...
		Raster[] result = null;
		// TODO: Implement this method, using copyXRange
		result = new Raster[manyNodes];
		if(isEmpty()) return result;
		copyXRange(result,0,root,lo,hi);
		result = trimFromNull(result);
		assert wellFormed() : "invariant false at the end of xRange()";
		return result;
	}
	...
	public Raster[] yRange(int lo, int hi) {
		assert wellFormed() : "invariant false at the start of yRange()";
		Raster[] result = null;
		// TODO: Implement this method, using copyYRange
		result = new Raster[manyNodes];
		if(isEmpty()) return result;
		copyYRange(result,0,root,lo,hi);
		result = trimFromNull(result);
		assert wellFormed() : "invariant false at the end of yRange()";
		return result;
	}
	...
	private int copyXRange(Raster[] array, int index, Node n, int lo, int hi) {
		...
		if(comp != BY_X || n.raster.x() >= lo) index = copyXRange(array,index,n.left,lo,hi);
		if (n.raster.x() <= hi && n.raster.x() >= lo) array[index++] = n.raster;
		if(comp != BY_X || n.raster.x() <= hi) index = copyXRange(array,index,n.right,lo,hi);
		return index;
	}
	...
	private int copyYRange(Raster[] array, int index, Node n, int lo, int hi) {
		...
		//   Otherwise, check every node.
		if(n == null) return index;
		if(comp != BY_Y || n.raster.y() >= lo) index = copyYRange(array,index,n.left,lo,hi);
		if (n.raster.y() <= hi && n.raster.y() >= lo) array[index++] = n.raster;
		if(comp != BY_Y ||n.raster.y() <= hi) index = copyYRange(array,index,n.right,lo,hi);
		return index;
	}
